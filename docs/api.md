...
M3a-Nachweisgrenze: Die GUI konsumiert vorhandene API-Faehigkeiten. Der aktuelle Full-Suite-Frontend-Truth-Lauf in `reports/archive/legacy/20260605T100000Z/frontend_full_suite_staged_report.json` und `reports/archive/legacy/20260605T100000Z/frontend_full_suite_staged_report.json` ist gruen (`82 collected`, `82 passed`, `0 failed`, `0 skipped`) und laeuft gegen echte API mit echter PostgreSQL-Testdatenbank. `reports/current/gate_hierarchy_result.json` steht auf `PASS`, Score `100.0`.
...
Der dokumentierte Zielzustand fuer M4a ist ein durchgaengig serverseitig aufgeloester Auth- und Workspace-Kontext fuer alle geschuetzten Pfade. Der technische Backend-Kern ist vorhanden; der browsernahe Auth-/Workspace-Frontend-Slice ist fuer M3a gruen belegt. M4a-Freigabeaussagen duerfen trotzdem nur aus den M4-Backend-Reports abgeleitet werden. Der aktuelle PostgreSQL Truth Report ist nicht gruen und blockiert M4/M5, aber nicht M3a.
