"""OpenAI provider for analysis summary generation."""
...
class OpenAiAnalysisProvider:
    ...
    def generate_analysis_summary(
        ...
        *,
        ...
        raw = self._post_with_retry(_OPENAI_API_URL, payload)
        return _parse_openai_response(raw, provider=self.provider_name, model=self.model_name)
            ...
            try:
                req = urllib.request.Request(
                    ...
                    headers={
                        ...
                        # API key intentionally not logged anywhere
                    },
                )
                ...
                last_exc = exc
                _log.warning("openai_timeout", extra={"attempt": attempt})
            except urllib.error.HTTPError as exc:
                _handle_http_error(exc, provider=self.provider_name, model=self.model_name)
            except urllib.error.URLError as exc:
                last_exc = exc
                _log.warning("openai_connection_error", extra={"attempt": attempt})

        if isinstance(last_exc, TimeoutError):
            raise ProviderTimeoutError(
                f"OpenAI timed out after {self.timeout_seconds}s",
...
def _handle_http_error(exc: urllib.error.HTTPError, *, provider: str, model: str) -> None:
    """Translate HTTP error codes to normalized provider errors."""
    if exc.code in (401, 403):
        raise ProviderAuthError(
            f"OpenAI rejected credentials (HTTP {exc.code})",
...
def _parse_openai_response(raw: dict, *, provider: str, model: str) -> AnalysisSummaryResult:
    try:
