...
def assert_no_truth_rows(session: Session) -> None:
    checks = {
        ...
        "chat_sessions": "select count(*) from chat_sessions where id like 'truth-%'",
        "background_jobs": "select count(*) from background_jobs where id like 'truth-%' or workspace_id::text like 'f1000000-%'",
    }
