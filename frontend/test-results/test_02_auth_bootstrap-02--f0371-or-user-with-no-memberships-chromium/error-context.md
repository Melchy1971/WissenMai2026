...
- Name: test_02_auth_bootstrap.spec.js >> 02 Auth bootstrap — 06 no workspace membership >> shows workspace-not-configured error for user with no memberships
- Location: tests\gui_truth\test_02_auth_bootstrap.spec.js:246:7
...
Call log:
  - waiting for locator('.state-card--error') to be visible
...
> 46  |   await page.waitForSelector('.state-card--error', { state: 'visible', timeout: 15_000 });
  ...
  136 |    * AuthContext sees no token → no bootstrap, straight to /login.
  137 |    */
  138 |   barePage: async ({ page }, use) => {
  139 |     await page.goto('/');
  140 |     await clearAuthState(page);
  141 |     await use(page);
  142 |   },
  143 | 
  144 |   credentials: async ({}, use) => {
  145 |     await use({
  146 |       login: process.env.TRUTH_LOGIN || 'gui_truth_user',
```