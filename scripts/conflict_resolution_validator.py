...
def resolve_conflicts():
    ...
    for fname in REQUIRED_REPORTS:
        ...
        if report.get("status") == "BLOCKED" or "error" in report:
            ...
            continue
        if report.get("generated_by") != "gate_validator":
            results[fname] = {"status": "BLOCKED", "error": f"{fname} was not generated by gate_validator"}
            continue
    ...
    if final and final.get("generated_by") != "gate_validator":
        return {"gate": "BLOCKED", "reason": "Final Release nicht aus Gate Validator Output erzeugt"}
...
if __name__ == "__main__":
    ...
    with open(CURRENT_DIR / "validation_report.json", "w", encoding="utf-8") as f:
